...
class DeepSpeechInterface {
  ...
  async generateLocalLM(tags) {
    ...
    const lmText = path.join(this._modelsDir, 'lm.txt');
    fs.writeFileSync(lmText, finalGrammar.join('\n'));

    const generateLm = spawnSync(
      'python3',
      [
        path.join(this._binDir, 'generate_lm.py'),
        '--input_txt', lmText,
        '--output_dir', this._modelsDir,
        '--top_k', '500000',
        '--kenlm_bins', this._binDir,
        '--arpa_order', '5',
        '--max_arpa_memory', '50%',
        '--arpa_prune', '0|0|1',
        '--binary_a_bits', '255',
    ...
    if (this._debug) {
      console.debug('generate_lm.py: status:', generateLm.status);
      console.debug('generate_lm.py: stdout:', generateLm.stdout.toString());
      console.debug('generate_lm.py: stderr:', generateLm.stderr.toString());
    }

    const generateScorer = spawnSync(
      path.join(this._binDir, 'generate_scorer_package'),
      [
        '--lm', path.join(this._modelsDir, 'lm.binary'),
        '--vocab', lmText,
        '--package', path.join(this._modelsDir, 'kenlm.scorer'),
        '--default_alpha', '0.931289039105002',
        '--default_beta', '1.1834137581510284',
        '--force_utf8', 'true',
        // https://github.com/mozilla/DeepSpeech/issues/3205
        '--alphabet', 'fake',
      ]
    );
    ...
    this._model = new Ds.Model(
      path.join(this._assetsDir, `deepspeech-model.${extension}`)
    );
    ...
    this._model.enableExternalScorer(
      path.join(this._modelsDir, 'kenlm.scorer')
    );
  ...
  resumeMicrophone() {
    ...
    const interimTimer = setInterval(() => {
      ...
      if (this._state === STATE.LISTENING_FOR_WAKE_WORD &&
          !transcript.endsWith(this._wakeWord)) {
        if (++silenceCount >= 10) {
          this.pauseMicrophone();
        }
      }
    ...
    const dataHandler = (data) => {
      dsStream.feedAudioContent(data.slice(0, data.length / 2));
    };
