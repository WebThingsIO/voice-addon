...
const fs = require('fs');
const {buildGrammar, matchTranscript} = require('./command-utils');
...
class DeepSpeechInterface {
  ...
  async generateLocalLM(devices) {
    ...
    const finalGrammar = [];
    const tags = devices.map((d) => d.title.toLowerCase());
    ...
    this._ws.on('message', (m) => {
      ...
      switch (this._state) {
        ...
        case STATE.LISTENING_FOR_COMMAND: {
          const match = matchTranscript(msg.transcript);
          if (match) {
            this._adapter.triggerEvent('speechinput', msg.transcript);
            // TODO: use it
            console.log(match);
            this._adapter.triggerEvent('command', msg.transcript);
            this.playSound('ding');
          } else {
