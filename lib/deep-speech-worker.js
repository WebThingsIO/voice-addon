...
class DeepSpeechWorker {
  ...
  loadModel() {
    ...
    if (DEBUG) {
      console.debug(`Loading model from ${this._modelRoot}`);
    }

    this._modelJson =
      JSON.parse(fs.readFileSync(path.join(this._modelRoot, 'info.json')));
    this._model =
      new Ds.Model(path.join(this._modelRoot, 'output_graph.tflite'), 500);

    if (DEBUG) {
      console.debug(`Enabling decoder on model ${this._modelRoot}`);
    }

    this._model.enableDecoderWithLM(
      path.join(this._modelRoot, 'local_lm.binary'),
