...
class DSAPIHandler extends APIHandler {
  constructor(addonManager) {
    ...
    this.setup().then((setupSuccess) => {
      ...
      if (!setupSuccess) {
         console.error('Setup failure, please verify logs with |sudo journalctl -xe -f -u mozilla-iot-gateway.service| to find more.');
         return;
      }
      ...
      // if the process it not yet listening, the WebSocket connection will fail,
      // the addon will crash and restart, and should be able to reconnect at
      // some point
      setTimeout(() => {
  ...
  async setup() {
    ...
    let downloadResult = await this.downloadModel(modelsDir);
