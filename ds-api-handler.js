...
class DSAPIHandler extends APIHandler {
  ...
  async setup() {
    const modelsDir =
      path.join(this.userProfile.dataDir, manifest.id, 'models');
    ...
    const downloadResult = await this.downloadModel(modelsDir);
  ...
  async generateLocalLM(devices) {
    ...
    const binDir =
      path.join(this.userProfile.baseDir, 'addons', manifest.id, 'bin');
  ...
  async handleRequest(request) {
    console.log(
      `request.method=${request.method} -- request.path=${request.path}`
    );
